...
# Library loading
####################################
if (!("discordr" %in% installed.packages()[,1])) {
  library(devtools)
...
library(pandoc)
#####################################
# THIS DOCUMENT WILL ALLOW YOU TO MAKE 
# POST GAME PITCHER REPORTS USING YAKKERTECH DATA. 
# IT COULD BE USED TO MAKE REPORTS FOR TRACKMAN DATA 
# AS WELL SINCE THE NAMING CONVENTIONS ARE VERY 
# SIMILAR, BUT MENTION OF COLUMN 'yt_Efficiency' 
# NEEDS TO BE REMOVED
#####################################

#####################################
# OPTIONAL - CREATE A DISCORD WEBHOOK FOR 
...
#####################################
#this loads the new CSV
#####################################
yak_23 <-  read.csv("sample csv.csv")
...
yakker$TaggedPitchType <- factor(yakker$TaggedPitchType, 
                                 levels = c("Fastball", 
                                            "Sinker", 
                                            "Cutter",
                                            "Curveball", 
...
# check and see how to CORRECT this
...
# creating tables and graphics for the
# report, and then rendering the report
...
for (pitcher in pitchers) {
  ...
  game_summary_table <- 
    ...
    dplyr:: mutate(TaggedPitchType = recode(
      TaggedPitchType, 
      Fastball = "FB", 
      Curveball = 'CB', 
      Sinker = 'SI', 
  ...
  stats_vs_r <- pitcher_data  %>%
    dplyr:: mutate(TaggedPitchType = 
                     recode(TaggedPitchType, 
                            Fastball = "FB", 
                            Curveball = 'CB', 
                            Sinker = 'SI', 
                            Slider = 'SL',
  ...
  ####################
  pitch_movement_plot_data <- pitcher_data %>%
    dplyr:: mutate(TaggedPitchType = recode(TaggedPitchType, Fastball = "FB", Curveball = 'CB', Sinker = 'SI', Slider = 'SL',
                                            Cutter = 'CT', Changeup = 'CH', Other = 'OT' ) )
  pitch_movement_plot <- 
     ggplot(data = pitch_movement_plot_data,
            aes(x = HorzBreak, 
                y = InducedVertBreak, 
  ...
  ######################
  # Pitch location plot vs rhh 
  # with a facet wrap on one of the 
  # created columns
  ######################
  plp_rhh_data <- pitcher_data |> 
    filter(
      # filter_col !='Take' & 
      #       filter_col !=''& 
      #       !is.na(filter_col), 
          BatterSide=='Right' ) |>
    dplyr:: mutate(TaggedPitchType = recode(TaggedPitchType, 
                                            Fastball = "FB", 
                                            Curveball = 'CB', 
                                            Sinker = 'SI', 
                                            Slider = 'SL',
  ...
  plp_rhh <- 
    ...
    ylim(0,4) + 
    labs(color = "",
         title = paste("Pitch Location vs RHH" )) +
    ...
    scale_color_manual(values = c('FB' = 'red', 
                                  'CB' = 'darkgreen', 
                                  'SI' = '#f47b20',  
                                  'SL'='cornflowerblue',
                                  'CT' = 'gold',  
  ...
  bd <- pitcher_data |>
    ...
    mutate(Count = paste0(Balls,"-",Strikes),
           filter_2 = ifelse(Count == "0-0", 
                             'FirstPitch',
                             ifelse(Strikes == 2, 
                                    '2 Strikes',
                                    ifelse(Count %in% c('1-0','2-0','3-0','2-1','3-1'), 
                                           'PitcherBehind',
                                           ifelse(Count %in% c('0-1','0-2', '1-2' ), 'PitcherAhead', ''
                                           ))  ))    
    )%>%
    filter(filter_2 != '') %>%
    # Next line throws a warning, CORRECT?
  ...
  input_file = "Scouting-Reports-P.Rmd"
